...
class ConvolutionalLayer(LayerWithParameters):
    ...
    def __init__(self, num_input_channels, num_output_channels,
        ...
        Args:
            ...
            biases_penalty: Biases-dependent penalty term (regulariser) or
                None if no regularisation is to be applied to the biases.
        """
        print(num_input_channels, num_output_channels)
    ...
    def fprop(self, inputs):
        ...
        outcome = np.zeros((inputs.shape[0],self.num_output_channels,output_height,output_width))
        out1=np.zeros((output_height,output_width))
        
        outchannel=self.kernels.shape[0]
        ...
        for i in range(inputs.shape[0]):
            for m in range(self.num_output_channels):
                ...
                for j in range(inputs.shape[1]):
                    x = inputs[i][j]  #xi j 
                    k = self.kernels[m][j]
                    temp = convolve2d(x,k, mode='valid')
                    temp=temp
                    out1=out1+temp
                outcome[i][m]=out1+self.biases[m]
#                 print("no-",i ,"k- ",m ,"i- ",i  ,":",outcome[i][m])
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        for i in range(batch_size):
            grad.append(grads_wrt_outputs[i])
        
        for i in range(len(grad)):
            for channel in range(self.kernels.shape[1]):
                k_temp = self.kernels[:, channel]
                grad_output_temp = grad[i]
                for b in range(batch_size):
                    x = grad_output_temp[b]
                    k = k_temp[b]
                    y_temp = correlate2d(x, k)
                    temp.append(y_temp)
        
        sub = []
        lll = len(temp)
        for i in range(0, lll-1, 2):
            y = temp[i] + temp[i+1]
            sub.append(y)
    ...
    def grads_wrt_params(self, inputs, grads_wrt_outputs):
        ...
        grads_wrt_kernels = self.kernels
        grads_wrt_biases = self.biases
        ...
        for index_batch in range(batch_size):
            grads_out_temp = grads_wrt_outputs[index_batch]
            for index_k in range(grads_out_temp.shape[0]):
                grads_temp = grads_out_temp[index_k]
                for index_inputs in range(self.num_input_channels):
                    in_temp = inputs[index_batch][index_inputs]
                    y_temp = correlate2d(in_temp, grads_temp, mode='valid')
                    ktemp[index_batch].append(np.rot90(y_temp, 2))
                    
        grads_wrt_kernels = np.add(k1temp, k2temp, dtype=float)
        grads_wrt_kernels = np.reshape(grads_wrt_kernels, self.kernels_shape)
        ...
        for index_batch in range(batch_size):
            for index_b in range(self.num_output_channels):
                b_temp[index_batch].append(grads_wrt_outputs[index_batch][index_b])
        for index_b in range(self.num_output_channels):
            b.append(np.add(np.sum(b0[index_b]), np.sum(b1[index_b]), dtype=float))
        grads_wrt_biases = np.reshape(b, self.num_output_channels)
...
class MaxPooling2DLayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        """
        b, c, height, width = inputs.shape
        poolheight = self.size
        poolwidth = self.size
        stride =  self.stride
        H_ = (height-poolheight)//stride+1
        W_ = (width-poolwidth)//stride+1
        out = np.zeros((b, c, H_, W_))
        for row in range(H_):
            for col in range(W_):
                out[:, :, row, col] = np.max(inputs[:, :, row*stride:row*stride+poolheight, col*stride:col*stride+poolwidth], axis=(2,3))
        return out
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        height = (H - self.size) // S + 1
        width = (W - self.size) // S + 1
            ...
            for P in range(C):
                for k in range(height):
                    for l in range(width):
                        xpooling = inputs[A, P, k *S : k * S + self.size, l * S : l * S + self.size]
                        maxi = np.max(xpooling)
                        xmask = xpooling == maxi
                        dd[A, P, k * S:k * S + self.size, l * S:l *S + self.size] += grads_wrt_outputs[A, P, k, l] * xmask
